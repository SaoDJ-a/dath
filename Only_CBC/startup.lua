...
local runCt = function()
    while true do
        ...
        if not properties.inversion then
            if tgPitch < properties.minPitchAngle then
                tgPitch = properties.minPitchAngle
                fire = false
            else
...
local runTerm = function()
    ...
    local selectBoxTb = {
        ...
        -- 新增以下内容 ▼
        inversion = newSelectBox(properties, "inversion", 2, 41, 14, false, true), -- X=41,Y=13位置添加反转开关
        -- 新增以上内容 ▲
    ...
    while true do
        ...
        if event == "mouse_up" or event == "key_up" or event == "alarm"
            ...
            if not alarm_flag then
                ...
                term.write("inwersi on") -- 在X=41,Y=13位置添加标签
                -- 新增以上内容 ▲
